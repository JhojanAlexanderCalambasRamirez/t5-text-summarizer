...
st.set_page_config(
    page_title="T5 - Resumen Automático de Texto",
    page_icon="T5",
    layout="wide",
...
with st.sidebar:
    ...
    num_beams = st.slider(
        ...
        help="1 = greedy decoding (rápido). Mayor número = mejor calidad pero más lento.",
    )
...
tab_demo, tab_attention, tab_architecture = st.tabs(
    ["Demo: Resumir texto", "Atención Cruzada (Q, K, V)", "Arquitectura T5"]
)
...
with tab_demo:
    ...
    if run_button:
        if not input_text.strip():
            ...
            m3.metric("Ratio de compresión",  f"{result['compression_ratio']}x")
...
# ===========================================================================
with tab_attention:
    st.subheader("Visualización de la Atención Cruzada")
    st.markdown(
        """
        La **atención cruzada** (*cross-attention*) es el mecanismo por el cual el decoder
        decide qué partes del texto de entrada son relevantes al generar cada token del resumen.

        | Tensor | Origen | Significado |
        |--------|--------|-------------|
        | **Q** (Query)  | Decoder | "¿Qué información necesito para generar este token?" |
        | **K** (Key)    | Encoder | "¿Qué información tengo disponible en la entrada?" |
        | **V** (Value)  | Encoder | "El contenido real que se extrae de la entrada" |

        El cálculo es: **Attention(Q, K, V) = softmax(QK^T / sqrt(d_k)) · V**
        """
    )
    st.divider()
    ...
    if "last_input" not in st.session_state:
        st.info("Primero genera un resumen en la pestaña **Demo** para visualizar la atención.")
    else:
        ...
        if st.button("Visualizar atención cruzada", type="primary"):
            ...
            ax.set_title(
                "Atención Cruzada - Última capa del Decoder (promedio de cabezas)",
                fontsize=12, pad=12,
            )
            ...
            st.caption(
                "Cada celda muestra cuánta atención pone el decoder en un token del encoder "
                "al generar el token de salida correspondiente. "
                "Colores más oscuros = mayor atención."
            )
...
with tab_architecture:
    ...
    with col_enc:
        ...
        st.markdown(
            ...
            **Capas por bloque:**
            ...
            2. **Feed-Forward Network** (2 capas lineales + activación)
    ...
    innovations = {
        "Framework Text-to-Text": (
            ...
            "traducción, resumen, clasificación, Q&A - solo cambia el prefijo de entrada."
        ),
        "Relative Position Bias": (
            "En lugar de sumar un encoding posicional absoluto a los embeddings, "
            "T5 añade un sesgo aprendido a los logits de atención basado en la "
        ...
        "Sin sesgos en capas densas": (
            "A diferencia del Transformer original, T5 elimina los términos bias "
            "en las proyecciones lineales Q, K, V, reduciendo parámetros sin perder calidad."
        ),
    ...
    st.markdown(
        """
        - **Idioma:** Los modelos base están entrenados principalmente en inglés (corpus C4).
        ...
        - **Longitud de contexto:** Limitado a 512 tokens de entrada en la configuración estándar.
        - **Sin memoria:** Cada inferencia es independiente; no recuerda ejecuciones anteriores.
        - **Sesgo del corpus:** C4 refleja sesgos presentes en la web en inglés.
        """
    )
