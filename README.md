...
El propósito central de esta investigación aplicada consiste en implementar un entorno funcional de inferencia utilizando la arquitectura Transformer encoder-decoder de T5, optimizada específicamente para la tarea de resumen automático de texto en inglés. Se busca estructurar un pipeline capaz de procesar secuencias lingüísticas de alta densidad conceptual sobre hardware de consumo general.

A nivel técnico y metodológico, el proyecto persigue el desarrollo de una interfaz interactiva basada en Streamlit que actúe como un laboratorio visual; a través de este módulo, se auditarán en tiempo real los coeficientes matemáticos de la matriz de atención cruzada mediante mapas térmicos bidimensionales configurables por capa y cabeza del decoder, se evaluará la calidad del resumen mediante métricas ROUGE y se demostrará la viabilidad del paradigma text-to-text validando la reducción de la latencia y la tasa de compresión bajo entornos de hardware limitados. Se utilizan como modelos principales `t5-small` y `t5-base`.
...
Bajo estas condiciones, se optó por la variante `t5-small` como opción por defecto. Con aproximadamente 60 millones de parámetros, esta variante produce resúmenes coherentes y gramaticalmente correctos, con tiempos de inferencia en CPU inferiores a 1 segundo por secuencia. La variante `t5-base` se ofrece como alternativa de mayor fidelidad semántica para entornos con mayor disponibilidad de memoria RAM.
...
| Pandas                   | >=2.0   | Estructuración tabular de resultados ROUGE y métricas comparativas del benchmark de modelos.               |
| rouge-score              | >=0.1.2 | Cálculo de métricas ROUGE-1, ROUGE-2 y ROUGE-L para evaluación de calidad del resumen generado.           |

### 4.3 Interfaz interactiva en Streamlit

Para democratizar el acceso al modelo y permitir la auditoría de los mecanismos internos de atención, se diseñó una interfaz gráfica interactiva dividida funcionalmente en cinco pestañas con una barra lateral de control global.

La barra lateral expone controles para parametrizar la variante del modelo y los hiperparámetros del pipeline generativo: longitud mínima y máxima de los tokens de salida, número de beams para beam search y penalización de longitud. Las cinco pestañas se organizan de la siguiente manera:

| Pestaña | Función principal |
|:--------|:------------------|
| Demo: Resumir texto | Ingreso del texto, generación del resumen y métricas de compresión con descarga JSON. |
| Atencion Cruzada: Layer / Head | Heatmap configurable por capa del decoder y cabeza de atención; interpretación automática de tokens más consultados. |
| ROUGE | Evaluación cuantitativa ROUGE-1, ROUGE-2 y ROUGE-L contra un resumen de referencia. |
| Comparacion de modelos | Benchmark comparativo de t5-small y t5-base sobre el mismo texto, con gráficos de tiempo de inferencia, tiempo por token y ratio de compresión. |
| Arquitectura T5 | Dimensiones reales del modelo cargado, descripción del encoder/decoder e innovaciones del paper. |

### 4.4 Extracción configurable de los pesos de atención cruzada
...
Matemáticamente, el tensor de atención cruzada devuelto por el modelo posee una forma cuatridimensional indexada por:

$$\mathcal{A} \in \mathbb{R}^{batch\\_size \times h \times target\\_seq\\_len \times source\\_seq\\_len}$$

La interfaz permite tres modalidades de extracción configurables mediante los parámetros `layer_idx`, `head_idx` y `average_layers`:

| Configuración | Descripción |
|:---|:---|
| `layer_idx=None`, `average_layers=False` | Última capa del decoder (comportamiento por defecto). |
| `layer_idx=N` | Capa específica $N$ del decoder, donde $0 \leq N < num\_layers$. |
| `average_layers=True` | Promedio aritmético sobre todas las capas del decoder. |
| `head_idx=None` | Promedio aritmético sobre todas las cabezas de atención. |
| `head_idx=H` | Cabeza específica $H$, donde $0 \leq H < num\_heads$. |

Para la visualización por defecto, la reducción del tensor se realiza mediante el promedio aritmético sobre las $h$ cabezas de la capa seleccionada:
...
Adicionalmente, el método `interpret_attention` promedia la matriz resultante por columna para identificar los tokens del encoder con mayor peso de atención acumulado a lo largo de toda la secuencia de salida, generando una interpretación textual automática de los tokens más influyentes.
...
├── inference.py            : Lógica T5: 4 métodos públicos (get_model_info, summarize,
│                              compute_rouge, get_cross_attention + interpret_attention)
├── app.py                  : Interfaz Streamlit de 5 pestañas
├── requirements.txt        : Dependencias CPU / Apple MPS
...
|:-----------------------------|:--------------------------------------|:-------------------------------|:--------------------------------|
| t5-small                     | Buena, preserva coherencia gramatical | ~0.5s por secuencia            | ~240 MB                         |
| t5-base                      | Muy alta fidelidad semántica          | ~2-4s por secuencia            | ~900 MB                         |

La evaluación comparativa evidencia la correlación existente entre el volumen de pesos neuronales, el tamaño del archivo binario en disco y las latencias de cómputo sobre hardware no acelerado. La variante `t5-base` exhibe capacidades superiores en la preservación de matices conceptuales complejos, pero su despliegue en CPU introduce latencias que comprometen la interactividad en tiempo real. La variante `t5-small` constituye el balance óptimo para demos educativas en CPU, con resúmenes coherentes y latencias por debajo de 1 segundo.
